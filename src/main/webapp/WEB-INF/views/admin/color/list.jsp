							...
							<table class="responsive-table highlight">
								<thead>
									<tr>
										<th>#</th>
										<th><a
											href="/admin/color?field=name&keywords=${keywords }&size=${size }">Material
												name</a></th>
										<th><a
											href="/admin/color?field=createdBy.fullName&keywords=${keywords }&size=${size }">Create
												by</a></th>
										<th><a
											href="/admin/color?field=createdDate&keywords=${keywords }&size=${size }">Create
												day</a></th>
										<th><a
											href="/admin/color?field=lastModifiedBy.fullName&keywords=${keywords }&size=${size }">Update
												by</a></th>
										<th><a
											href="/admin/color?field=lastModifiedDate&keywords=${keywords }&size=${size }">Update
												day</a></th>
										<th>Action</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="item" items="${resultPage.content }"
										...
										<tr>
											...
											<td><c:set var="colorCss" value="${item.name }-text" /><label
												class="mt-4"> <span><i
														class="material-icons vertical-align-bottom ${colorCss }">
															lens </i> ${item.name }</span>
											</label></td>
											...
											<td>
												<div class="invoice-action">
													<a href="#modalViews" data-id="${item.id}"
														onclick="viewApiColor(this.getAttribute('data-id'))"
														...
														onclick="deleteColor(this.getAttribute('data-id'), this.getAttribute('data-name'))"
											...
											<aside id="right-sidebar-nav">
												<ul id="${item.id}"
													...
													<li class="chat-body">
														<div class="card-content">
															<form:form action="/admin/color/saveOrUpdate"
							...
							<c:if test="${resultPage.totalPages > 0}">
								<ul class="pagination center-align">
									<li
										...
										<a
										href="/admin/color?field=${field }&keywords=${keywords }&size=${size }&page=1"><i
									...
									<c:forEach var="pageNumber" items="${pageNumbers }">
										<c:if test="${resultPage.totalPages > 1}">
											<li 
												...
												<a
												href="/admin/color?field=${field }&keywords=${keywords }&size=${size }&page=${pageNumber }">${pageNumber}
											</a>
											</li>
										</c:if>
									</c:forEach>
									...
									<li
										...
										<a
										href="/admin/color?field=${field }&keywords=${keywords }&size=${size }&page=${resultPage.totalPages }"><i
